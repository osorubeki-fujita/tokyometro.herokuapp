def db_update_to_station_id_of_namboku_line_train_bound_for_musashi_kosugi
  train_times = ::TrainTime.where( id: db_train_time_ids_of_namboku_line_bound_for_musashi_kosugi )
  to_station_id_of_meguro_musashi_kosugi = ::ToStation.find_by( station_id: ::Station.find_by( same_as: "odpt.Station:Tokyu.Meguro.MusashiKosugi" ).id ).id
  train_times.each do | train_time |
    train_time.update( to_station_id: to_station_id_of_meguro_musashi_kosugi )
  end
end
