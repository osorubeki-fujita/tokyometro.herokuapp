...
def db_train_time_ids_of_chiyoda_line_kitasenju
  timetable_ids = ::Timetable.where(
    ...
    station_id: ::Station.where( name_ja: "北千住" ).pluck( :id )
  )
...
def db_train_time_ids_of_namboku_line_bound_for_musashi_kosugi
  ...
  musashi_kosugi_ids = ::Station.where( name_in_system: "MusashiKosugi" ).pluck( :id )
  to_station_ids = ::ToStation.where( id: musashi_kosugi_ids ).pluck( :id )
  ::TrainTime.where( timetable_id: namboku_line_timetable_ids , to_station_id: to_station_ids ).pluck( :id )
end