...
%div{ class: :another_station }
  %div{ class: :text_ja }<
    = decorator.render_name_ja( with_subname: false , suffix: this.attribute_ja )
  %div{ class: :text_en }<
    = decorator.render_name_en( with_subname: false , suffix: this.attribute_en_short.capitalize )
    HAML
  end
