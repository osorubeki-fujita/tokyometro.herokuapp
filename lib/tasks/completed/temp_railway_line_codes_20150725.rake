namespace :temp do
  ...
  namespace :completed do
    ...
    task :railway_line_codes_20150725 => :environment do
      ::Railway::Line::Code.all.to_a.each do | item | ; item.destroy ; end
        ::Railway::Line::InfoCode.all.to_a.each do | item | ; item.destroy ; end
      ...
      for i in 0..( railway_line_infos.length - 1 )
        ...
        when "odpt.Railway:Tobu.SkytreeIsesaki"
          ...
          code_1 = ::Railway::Line::Code.find_or_create_by( code: "TS" , color: "\#0f6cc3" )
          code_2 = ::Railway::Line::Code.find_or_create_by( code: "TI" , color: "\#ff0000" )
          [ code_1 , code_2 ].each.with_index(1) do | code_info , i |
            ::Railway::Line::InfoCode.find_or_create_by( info_id: railway_line_info.id , code_id: code_info.id , index: i )
          end
          ...
          code_new = ::Railway::Line::Code::find_or_create_by( code: code , color: color_info )
          ::Railway::Line::InfoCode.find_or_create_by( info_id: railway_line_info.id , code_id: code_new.id , index: 1 )
        end
