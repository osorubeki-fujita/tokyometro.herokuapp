# 列車運行情報 odpt:TrainInformation を扱うクラスのリスト
module TokyoMetro::ClassNameLibrary::Api::TrainInformation

  extend ::ActiveSupport::Concern

  module ClassMethods

    # @!group クラスメソッド (1) - メタデータ

    # クラス指定 - odpt:TrainInformation
    # @return [String]
    def rdf_type
      "odpt:TrainInformation"
    end

    # JSON-LD 仕様に基づく context のURL - URL
    # @return [String]
    def context
      "http://vocab.tokyometroapp.jp/context_odpt_TrainInformation.json"
    end

    # @!group 生成するクラスの情報

    # トップレベルのクラス
    # @return [Const ( ::TokyoMetro::Api::TrainInformation )]
    def toplevel_namespace
      ::TokyoMetro::Api::TrainInformation
    end

    # 配列のクラス
    # @return [Const ( ::TokyoMetro::Api::TrainInformation::List )]
    def list_class
      ::TokyoMetro::Api::TrainInformation::List
    end

    # 配列の要素となるインスタンスのクラス
    # @return [Const ( ::TokyoMetro::Api::TrainInformation::Info )]
    def info_class
      ::TokyoMetro::Api::TrainInformation::Info
    end

    # @!group Factory Pattern のクラスの情報

    # API からデータを取得するための Factory Pattern クラス
    # @return [Const ( ::TokyoMetro::Factories::Api::Get::DataSearch::TrainInformation )]
    def factory_for_getting
      ::TokyoMetro::Factories::Api::Get::DataSearch::TrainInformation
    end

    # API から取得したデータを保存するための Factory Pattern クラス
    # @return [Const ( ::TokyoMetro::Factories::Api::Save::DataSearch::TrainInformation )]
    def factory_for_saving
      ::TokyoMetro::Factories::Api::Save::DataSearch::TrainInformation
    end

    # グループ化されたデータを保存するための Factory Pattern クラス
    # @return [Const ( ::TokyoMetro::Factories::Api::SaveGroupedData::TrainInformation )]
    def factory_for_saving_datas_of_each_group
      ::TokyoMetro::Factories::Api::SaveGroupedData::TrainInformation
    end

    # JSON をパースして得られた配列の要素である Hash からインスタンスを作成するメソッドための Factory Pattern クラス
    # @return [Const ( ::TokyoMetro::Factories::Api::GenerateFromHash::TrainInformation )]
    def factory_for_generating_from_hash
      ::TokyoMetro::Factories::Api::GenerateFromHash::TrainInformation
    end

    # 保存済みの情報を処理しインスタンスを復元するための Factory Pattern クラス
    # @return [Const ( ::TokyoMetro::Factories::Api::GenerateFromSavedFile::TrainInformation )]
    def factory_for_generating_from_saved_file
      ::TokyoMetro::Factories::Api::GenerateFromSavedFile::TrainInformation
    end

    private

    # @!group クラスメソッド - データの取得・保存

    # データを保存するディレクトリ
    # @return [String]
    def db_dirname_sub
      "train_information"
    end

  end

end