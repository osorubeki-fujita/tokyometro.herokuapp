...
class TokyoMetro::StaticDatas::StationsInTokyoMetro::Info
...
# @!group 管理する鉄道事業者に関するメソッド
  ...
  # @return [::Array <TokyoMetro::StaticDatas::Operator::Info>] 管理事業者
  attr_reader :administrator

  # @return [::Array <TokyoMetro::StaticDatas::Operator::Info>] その他の乗り入れ事業者
...
# @!group インスタンスの基本的な情報を取得するメソッド
  ...
  def to_s( indent = 0 )
    self.instance_variables.map { |v|
      ...
      when :@administrator , :@other_operator
        val = val.map { | operator | operator.same_as }.join("／")
      else
        if val.instance_of?( ::Array )
          val = val.map { | element | element.to_s }.join("／")
        else
          val = val.to_s
        end
      end
  ...
  def set_operator_name( info )
    ...
    ary_of_info.map { | operator |
      # 事業者のインスタンスを取得
      instance_of_operator = ::TokyoMetro::StaticDatas.operators[ operator ]
      # ハッシュの返り値が nil の場合
      if instance_of_operator.nil?
        ...
        if regexp === operator
          ...
          instance_of_operator = ::TokyoMetro::StaticDatas.operators[ operator_name ]
        end
      end
      # やり直しても返り値が nil の場合
      if instance_of_operator.nil?
...
# @!group クラスメソッド
  ...
  def self.generate_from_hash( same_as , h )
    ary_of_keys = [ :station_facility_custom , :station_facility_custom_alias , :name_ja , :name_hira , :name_in_system , :name_en ,
