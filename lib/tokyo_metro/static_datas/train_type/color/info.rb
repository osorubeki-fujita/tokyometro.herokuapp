# 個別の種別の色を扱うクラス
class TokyoMetro::StaticDatas::TrainType::Color::Info < TokyoMetro::StaticDatas::Color
...
# @!group 種別色の ID ・補足情報に関するメソッド
  ...
  # @example
  #   TokyoMetro::StaticDatas.train_types_color.each_value { | color | puts color.same_as }
  #   =>
  ...
  # @example
  #   TokyoMetro::StaticDatas.train_types_color.each_value { | color | puts color.note }
  #   =>
  #   一般 標準色 白
  #   一般 標準色 グレー
  #   土休急行 標準色（ピンク）
...
# @!group 種別色の値を取得するメソッド - 例を追加するためだけに定義
  ...
  # @return [String]
  # @example
  #   TokyoMetro::StaticDatas.train_types_color.each_value { | color | puts color.same_as.ljust(48) + " " + color.web }
  #   =>
  ...
  # @return [String]
  # @example
  #   TokyoMetro::StaticDatas.train_types_color.each_value { | color | puts color.same_as.ljust(48) + " " + color.rgb_in_parentheses }
  #   =>
...
# @!group クラスメソッド
  ...
  def self.generate_from_hash( same_as , h )
    note = generate_from_hash__set_variable( h , :note )
    ary = generate_from_hash__set_color_info( h )
    self.new( same_as , note , *ary )
  end
