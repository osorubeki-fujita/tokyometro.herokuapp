...
class TokyoMetro::StaticDatas::Fare::Normal
  ...
  def initialize
    @tables = ::Array.new
    @tables << self.class.normal_fare_table_class.generate_from_yaml( "Fare" , "20140401_normal" , self.class.static_datas_toplevel_namespace.last_revision ).freeze
  end

  # データベースをもとにした運賃表（現在）
  # @return [::Array <::TokyoMetro::StaticDatas::Fare::Normal::Table>]
  attr_reader :tables
  ...
  def self.seed
    TokyoMetro::Seed::Inspection.title( self )
    time_begin = ::Time.now
