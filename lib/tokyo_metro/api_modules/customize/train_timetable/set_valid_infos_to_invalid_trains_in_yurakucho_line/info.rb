...
module TokyoMetro::ApiModules::Customize::TrainTimetable::SetValidInfosToInvalidTrainsInYurakuchoLine::Info

  # 「本来は副都心線の時刻表であるが、有楽町線の時刻表に含まれてしまっている列車か否か」を判定するメソッド
  # @return [Boolean]
  # @note 運行日が休日であることに注意
  def invalid_fukutoshin_line_trains_on_yurakucho_line?
    yurakucho_line? and operated_on_weekdays? and ::TokyoMetro::ApiModules::Customize::TrainTimetable.invalid_trains_in_yurakucho_line.include?( @train_number )
  end

  def valid_fukutoshin_line_train_info_as_for_invalid_train_in_yurakucho_line( train_number_of_invalid_train )
    fukutoshin_line? and is_operated_on_weekdays? and @train_number == train_number_of_invalid_train
  end
