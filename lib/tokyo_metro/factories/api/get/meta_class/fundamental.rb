...
class TokyoMetro::Factories::Api::Get::MetaClass::Fundamental

  include ::ForRails::CommonModules::InspectProcessTime
  ...
  def get_data( http_client )
    ...
    if @to_inspect
      # progress_bar.download_completed!( title )
      inspect_process_time( "Download" , time_begin )
      puts ""
    end
  ...
  def generate_new_array_instance( ary )
    ...
    list_class.new( ary.map { | element | info_class.generate_from_hash( element ) } )
  end
