module TokyoMetro::Seed::TrainTypeScss
  ...
  class << self
    ...
    def dir_of_stylesheets
      ::File.expand_path( "#{File.dirname( __FILE__ ) }/../../../../rails_tokyo_metro/app/assets/stylesheets" )
    end
    ...
    def main_content_document_train_type( train_types )
      ...
      train_types_grouped_by_operator.keys.sort.each do | operator_id |
        ...
        train_types_grouped_by_railway_line.keys.sort.each do | railway_line_id |
          ...
          raise "Error: #{operator_instance.same_as} - The railway line \##{railway_line_id} is not defined." if railway_line_instance.nil?
          ary << begin_scss_content( ".#{ railway_line_instance.css_class_name }" , indent , 2 )
