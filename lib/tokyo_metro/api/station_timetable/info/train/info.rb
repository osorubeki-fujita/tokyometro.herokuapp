# 個別の列車の情報を扱うクラス
class TokyoMetro::Api::StationTimetable::Info::Train::Info
  ...
  def start_from_this_station?
    @is_origin or @notes.any? { |i| i.instance_of?( ::TokyoMetro::Api::StationTimetable::Info::Train::Info::Note::StartFromThisStation ) }
  end

  def depart_from
    info = @notes.find { |i| i.instance_of?( ::TokyoMetro::Api::StationTimetable::Info::Train::Info::Note::DepartFrom ) }
    if info.nil?
      nil
    else
      info.number
  ...
  # @param change_day_of_midnight_train [Boolean] false の場合は日付の変更を行わない。（default は true だが、::TokyoMetro::Api::StationTimetable::Info::Train::List#last_train_not_departed_yet? では false に設定している）
  # @return [DateTime]
  # @note 時刻の比較を簡単にする。
  ...
  def bound_for?( *stations )
    if stations == [ nil ]
      ...
      stations.any?{ | station |
        ( station.string? and station == @terminal_station ) # or station.instance_of?( Station and station.name = @terminal_station )
      }
    end
  end
  ...
  def check_class_of_variable_when_initialize( departure_time , terminal_station , train_type ,
    is_last , is_origin , car_composition , notes )
    nil
  end
  private :check_class_of_variable_when_initialize
...
# @!group クラスメソッド (2) - データの取得・保存
  ...
  def self.generate_from_hash(h)
    super( h , factory_name: :factory_for_generating_train_from_hash )
  end
