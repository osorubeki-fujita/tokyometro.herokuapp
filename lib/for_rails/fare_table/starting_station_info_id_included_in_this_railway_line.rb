...
module ForRails::FareTable::StartingStationInfoIdIncludedInThisRailwayLine
  ...
  def fare_table_starting_station_info_id_included_in_this_railway_line( station_infos_of_this_instance )
    ids_of_station_infos = station_infos_of_this_instance.pluck( :id )
    ids_of_this_railway_line = self.station_infos.pluck( :id )
    ary = ( ids_of_station_infos & ids_of_this_railway_line )
    if ary.empty?
      nil
    elsif ary.length == 1
      ary.first
